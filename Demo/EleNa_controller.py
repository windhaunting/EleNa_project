...
class EleNa_Controller(object):
    ...
    def get_dist_cost(self, graph, node_a, node_b):
        return graph.edges[node_a, node_b, 0]['length']
    
    def get_elevation_cost(self, graph, node_a, node_b):
        # get each 
        return (graph.nodes[node_a]['elevation'] - graph.nodes[node_b]['elevation'])
    
    
    def get_total_elevation(self, graph, route):
        # input: route
        # output: total evelvation
        if not route:
            return 0
        elevation_cost = 0
        for i in range(len(route)-1):
            elevation_data = self.get_elevation_cost(graph, route[i], route[i+1])
            if elevation_data > 0:
    ...
    def get_distance_length_by_route(self, graph, route):
        # intput: route
        # output: the length
        n = len(route)
        total_dist = 0 
        for i in range(0, n-1):
            node_a = route[i]
            node_b = route[i+1]
            total_dist += graph.edges[node_a, node_b, 0]['length']
        return total_dist
    ...
    def plot_three_routes(self, graph, route1, route2, route3,  src_lat_long, destination_lat_long):
        #self.plot_path(lat, long, src_lat_long, destination_lat_long)
        ...
        # plot the routes
        fig, ax = ox.plot_graph_routes(graph, [route1, route2, route3], route_color=rc, orig_dest_node_color=nc, node_size=0)
     
        
    def plot_two_route(self, graph, route1, route2, src_lat_long, destination_lat_long, filename):
        rc1 = ['r'] * (len(route1) - 1)
        rc2 = ['b'] * len(route2)
        rc = rc1 + rc2
        nc = ['r', 'r', 'b', 'b']
        fig, ax = ox.plot_graph_routes(graph, [route1, route2], 
            route_color=rc, orig_dest_node_color=nc, node_size=0,
            show=False, close=False , save=True, filename=filename)
    ...
    def plot_html(self, source_X, source_Y, sink_X, sink_Y, route, mode):
        ...
        graph_project, graph_orig = self.read_map_data(False, graph_origin_file, graph_project_file)
    ...
    def cal_route_dij(self, source_X, source_Y, sink_X, sink_Y, Tol_dir, Tol_val):
        src_lat_long = (source_X, source_Y)
        destination_lat_long = (sink_X, sink_Y)

        graph_origin_file = "data/Amherst_city_graph.pkl"
        graph_project_file = "data/Amherst_city_graph_projected.pkl"
        graph_project, graph_orig = self.read_map_data(False, graph_origin_file, graph_project_file)
        ...
        shortest_path_length = self.get_ground_truth_shorest_length(graph_orig, source, destination)
        overhead = float(Tol_val)
        ...
        self.plot_html(source_X, source_Y, sink_X, sink_Y, route1, "Best")
        self.plot_html(source_X, source_Y, sink_X, sink_Y, route2, Tol_dir)
        self.plot_two_route(graph_orig, route1, route1, src_lat_long, destination_lat_long, "Best")
        self.plot_two_route(graph_orig, route2, route2, src_lat_long, destination_lat_long, Tol_dir)
    ...
    def test2(self):
        ...
        print("edges: ", len(graph_orig.edges(data=True)))
        print("edges info: ", list(graph_orig.edges(data=True))[0])
